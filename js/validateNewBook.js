$(document).ready(function() {
    ...
    function validateCopies() {
        const copies = copiesInput.val();
        const copiesErrorDiv = $('#copyErr');
        if (!/^[0-9]*$/.test(copies)) {
            copiesErrorDiv.html('Number of copies should contain only digits.');
            copiesErrorDiv.css("color", "red");
            return false;
        } else if (copies <= 0) {
            copiesErrorDiv.html('Number of copies should be greater than zero.');
            copiesErrorDiv.css("color", "red");
            return false;
        } else {
            copiesErrorDiv.html('');
